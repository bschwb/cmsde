...
def train_neural_network(sess):
    ...
    for m in range(0, M):
        ...
        if m % int(M/100) == 0:
            ...
            E_a.append(cost.eval(feed_dict={inp:x_training}))
        if m % int(M/10) == 0:
            print(m)

    return alpha, E_1, E_a

pts = np.linspace(-4,4,300).reshape(-1,1)

with tf.Session() as sess:
    alpha, E_1, E_a = train_neural_network(sess)
...
plt.semilogy(np.linspace(0, T, len(E_a)), E_a, label='E_a')
plt.legend()
plt.savefig('pic/exp_vs_emp_loss_overfit.pdf', bbox_inches='tight')
plt.show()
