...
def neural_network_model(inp):
    ...
    hidden_layer_1 = tf.layers.dense(inputs=inp,
                                     ...
                                     bias_initializer=bias_initializer(
                                         minval=20*np.pi, maxval=22*np.pi))
...
plt.legend()
plt.savefig('pic/learned_fun_sin.pdf'.format(dt), bbox_inches='tight')
# plt.show()

plt.figure('Test error', figsize=(15,10));
plt.semilogy(np.linspace(0, T, len(E_1)), E_1, label='E_1')
plt.legend()
plt.savefig('pic/test_error_sin.pdf'.format(dt), bbox_inches='tight')
# plt.show()
