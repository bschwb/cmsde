...
The Euler discretization of $\int_0^T{W_t \dW}$ is $\Sm{W(t_{n})\D W_n}$.
...
\begin{IEEEeqnarray*}{rCl}
  ...
  & = & \Sm{(W(t_{n+1}) + W(t_{n}))(W(t_{n+1}) - W(t_{n}))} \\
  & = & \Sm{W(t_{n+1})^2 - W(t_n)^2} \\
  & = & W(T)^2 - W(0)^2 = W(T)^2 \\
\end{IEEEeqnarray*}
The variance for the increments of a Wiener process are normaly distributed with
variance $\EV{(\D W_n)^2} = \D t_n$. Thus follows:
\[ \EV{\Sm{(\D W_n)^2}} = \Sm{\D t_n} = T\]
The kurtosis for the normal distribution is $3$ so $\EV{(\D W_n)^4} = 3 (\D
t_n)^2$.
The independence of the increments together with $\EV{(\D W_n)^2 - \D t_n} = 0$
leads to:
\[
